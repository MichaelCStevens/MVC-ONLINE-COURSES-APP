 ...
 * Paybook Model
 */
class myaccountModelseminars extends JModelItem {
   ...
   public function deleteItem($id) {
        ...
        $query = "DELETE FROM seminars_users WHERE seminar_id='$id'";
        $this->db->setQuery($query);
        $this->db->query();
    }
    ...
    public function getSeminars() {
        $query = "SELECT a.*, b.title as catTitle FROM seminars as a
    ...
    public function updateSeminar($id, $post) {
        ...
        $query = "UPDATE seminars 
              SET cat='" . $post['cat'] . "', title='" . addslashes($post['title']) . "', hours='" . $post['hours'] . "',price='" . $post['price'] . "',email_text='" . addslashes($post['email_text']) . "',description='" . addslashes($post['description']) . "', publish='" . $post['publish'] . "'
                WHERE id='$id'";
        $this->db->setQuery($query);
        $this->db->query();
               return 'seminar Updated';
    }

    public function publishSeminar($p, $id) {
        if ($p == 2) {
            $p = 0;
        }
        $query = "UPDATE seminars SET publish='$publish' WHERE id='$id'";
        $this->db->setQuery($query);
        $this->db->query();
        return 'seminar Updated';
    }

    public function newSeminar($post) {


//        $material = $this->uploadFile($post['material'], '/components/com_myaccount/uploads/seminars/');
...
//        }
        $query = "INSERT INTO  seminars 
           SET cat='" . $post['cat'] . "', title='" . addslashes($post['title']) . "', hours='" . $post['hours'] . "',price='" . $post['price'] . "',description='" . addslashes($post['description']) . "', publish='" . $post['publish'] . "'";
        $this->db->setQuery($query);
        $this->db->query();
        return 'seminar "' . $post['title'] . '" created';
    }
